...
paperkey_print_armour() {
    for FILE; do
        lp -o media=a4 -o page-bottom=25 -o page-left=25 -o page-right=10 -o page-top=10 -o sides=two-sided-long-edge "$FILE"
    done
}

paperkey_print_all() {
    echo "Print both paperkey of GPG secret keys + public keys as armoured ASCII"
    paperkey_print *.paperkey
    paperkey_print_armour *.pub.asc
}
